...
public struct SwiftPM: Sendable {
  ...
  public let clangModuleCachePath: URL
  ...
  public init(workingDirectory: URL, stateDirectory: URL? = nil) throws {
    ...
    let supportDirectory = stateDirectory
      ?? FileManager.default.temporaryDirectory
        .appendingPathComponent("FontAnatomySwiftPM-\(UUID().uuidString)", isDirectory: true)
    ...
    clangModuleCachePath = supportDirectory.appendingPathComponent(
      "clang-module-cache",
      isDirectory: true
    )

    try [scratchPath, cachePath, configPath, securityPath, clangModuleCachePath].forEach {
  ...
  private var environment: [String: String] {
    [
      "CLANG_MODULE_CACHE_PATH": clangModuleCachePath.path
    ]
  }
}
